...
class DrawingOverlay(QMainWindow):
    ...
    def keyPressEvent(self, event):
        if self.is_selecting_region and event.key() == Qt.Key_Escape: self.cancel_screenshot(); return
    ...
    def _perform_save(self, crop_rect):
        ...
        if success: self.show_toast(f"Kaydedildi: {save_path}")
    ...
    def take_screenshot(self):
        ...
        self.is_selecting_region = True; self.select_start = QPoint(); self.select_end = QPoint(); self.setCursor(Qt.CrossCursor); self.show_toast("Alan Seçin"); self.update()
    def cancel_screenshot(self): self.is_selecting_region = False; self.setCursor(Qt.ArrowCursor); self.toolbar.show() if self.toolbar else None; self.update()
